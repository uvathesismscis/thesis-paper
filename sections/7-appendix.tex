...
A floorplan diagram providing an overview of the technical lab setup of the small (Room A) and large (Room B) meeting rooms used for indoor air quality monitoring. The floorplan indicates the position the monitors are installed and the position of doors and windows.
...
Photographs of the small (Room A) and large (Room B) meeting rooms taken from the viewpoint of the entrance of the door. It shows the default interior design set-up of the rooms and the position of the chairs and tables.
...
Photographs of the prototype-creating phase showing the manufacturing of the 3D printed components, connecting the electronics, writing the firmware and digitally modeling the components of the hi-fi data physicalization. 
...
System architecture diagram which shows the Internet of Things (IoT) architecture of the prototype following the notion of edge computing using a 1) sensing layer, 2) networking layer, 3) processing layer and 4) application layer.
...
Plots for specific questions of the survey questionnaire data exported from Qualtrics. Bar charts are created to show the distribution of answers for each question. The Likert scales are plotted as a stacked bar chart to show the distribution of scores.
...
Plots of the indoor air quality monitor data logs. All graphs show the CO2 concentrations in either the small (Room A) or large (Room B) meeting rooms. The plots are subsets of the data logs which show interesting patterns and peaks based on the observed timeframe of the data collection phase.
...
Sample size of the participants who participated in the evaluation sessions with their expertise for demographic purposes.
...
Coding of design improvements and recommendations based on the thinking aloud of participants during the usability testing.
...
Individual results of the statement scores based on the Likert-scale questionnaire in both a table with variances and bar chart plot.
...
In the ideation phase and development of the concept models an exploration of 28 non-academic case studies were instrumental in informing the design of the prototype. \hyperref[tab:non-academic-cases]{Table \ref{tab:non-academic-cases}} provides an overview of all these case studies.
